...
export default async function MovieDynamicPage({ params }) {
  ...
  return (
    <div classname='w-full'>
      <div className='p-4 md:pt-8 flex flex-col lg:flex-row items-center content-center max-w-6xl mx-auto md:space-x-6'>{(movie.backdrop_path || movie.poster_path) ?
        ...
        <div className='p-2'>
            ...
            <p className='text-lg mb-3'><span className='font-semibold mr-1'>Overview:</span> {movie.overview || 'No overview yet'}</p>
            <p className='mb-3'><span className='font-semibold mr-1'>Date Released:</span> {movie.release_date || movie.first_air_date}</p>
            <p className='mb-3'><span className='font-semibold mr-1'>Rating:</span> <span className='font-semibold'>{movie.vote_average !== undefined ? movie.vote_average.toFixed(2) : 0 }</span> of {movie.vote_count} votes</p>
        </div>
      </div>
    </div>
  )
