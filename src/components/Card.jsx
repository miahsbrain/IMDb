  ...
  return (
    <div className='cursor-pointer sm:p-3 sm:hover:shadow-slate-500 shadow-md rounded-lg sm:border sm:border-slate-500 sm:m-2 transition-shadow transition-duration-200 group'>
        <Link href={`/movie/${result.id}`}>
            <Image src={`https://image.tmdb.org/t/p/original${result.backdrop_path || result.poster_path}`} width='640' height='300' alt={result.original_title || 'Image not found'} className='sm:rounded-t-lg group-hover:opacity-80 transition-opacity transition-duration-200' placeholder='blur' blurDataURL='/spinner.svg' style={{height:'auto', maxWidth: '100%'}} />
            <div className='p-2'>
